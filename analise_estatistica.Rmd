...
O estudo “Resposta Terapêutica na Malária por Plasmodium vivax: Variabilidade Genética de Enzimas Metabolizadoras da Primaquina e o Clearance de Gametócitos” é uma pesquisa transversal desenvolvida em colaboração com um programa de pós-graduação em Ciências da Saúde, com apoio de uma fundação de renome na área de saúde. Este estudo visa analisar a frequência de polimorfismos nos genes CYP2D6 e POR e investigar como essas variações genéticas afetam a atividade da Primaquina, um medicamento utilizado na prevenção da malária, e o clearance de gametócitos em infecções por P. vivax. O objetivo é determinar se há evidências estatísticas de que polimorfismos nos genes CYP2D6 e POR influenciam a eficácia da Primaquina.
...
1. Razão de Pvs 25 (Eliminação de gametócitos): Mede a quantidade de gametócitos femininos maduros após 72 horas em relação ao momento inicial. Em escala logarítmica (base 10), varia de -5,000 a 3,848, onde valores mais altos indicam maior quantidade de gametócitos após 72 horas.

2. Concentração de Primaquina (PQCONC): Quantifica a concentração do fármaco Primaquina no momento final da coleta. Varia de 1,431 a 2,565, com valores mais altos indicando maior concentração de Primaquina.

3. Quantificação de Parasitos:
- PV18SD0NORM: Quantidade de parasitos no momento inicial, variando de -5,000 a 7,327. Valores mais altos indicam maior concentração de parasitos. 
- PV18SD72NORM: Quantidade de parasitos após 72 horas, variando de -5,000 a 5,292.

4. Status de Metabolização: 
- CYP2D6: Classificado como "Normal" ou "Alterada".
- POR: Classificado como "Mutada" ou "Não-mutada".
- CYP2D6/POR Conjunto: Classificado em quatro categorias:"Normal/Não-mutada", "Normal/Mutada", "Alterada/Não-mutada", "Alterada/Mutada".
- Fenótipo CYP2D6: Classificado como "gPM" (metabolizador nulo), "gIM" (metabolizador intermediário), "gNM-S" (metabolização reduzida), "gNM-F" (metabolização normal), e "gUM" (metabolização ultrarrápida).

6. Status dos Gametócitos Masculinos:
- PVS48D0: Presença ou ausência de gametócitos masculinos no início, classificado como "Positivo" ou "Negativo".
- PVS48D72: Presença ou ausência de gametócitos masculinos após 72 horas, classificado como "Positivo" ou "Negativo".

## Análise Exploratória

Realizada com objetivo de conhecer as características e identificar padrões nos dados. Parte desse processo foi desenvolvido por meio da obtenção de medidas resumo (média, mediana, desvio padrão, máximo e mínimo), para as variáveis numéricas, e frequência relativa de cada um dos níveis das variáveis categóricas. Além disso, foram elaborados gráficos de dispersão, boxplots e histogramas visando um melhor entendimento das medidas de posição, tendência central e da disperão dos dados.
...
Devido à natureza sensível dos dados utilizados, que envolvem informações de saúde de indivíduos participantes do estudo. Nese tópico de resultados será apresentado apenas o código, sem a interpretação dos resultados.
...
Gráficos de dispersão para visualização da relação entre as variáveis Razão de Pvs25 (Eliminação de gametócitos) e Quantidade de Parasitos (0 horas), com relação aos status de metabolização conjunto e individuais:
...
Visando testar estatisticamente se exite uma associação entre as variáveis de interesse, postulou-se testes de independencia qui-quadrado, teste de correlação e testes de médias/medianas, os quais possuem as seguintes hipótese de interesse:
...
shapiro.test(bd$PV18SD0NORM[bd$CYP2D6POR == "AM"]) 
shapiro.test(bd$PV18SD0NORM[bd$CYP2D6POR == "AN"]) 
shapiro.test(bd$PV18SD0NORM[bd$CYP2D6POR == "NM"]) 
shapiro.test(bd$PV18SD0NORM[bd$CYP2D6POR == "NN"])

##############################
#cyp
shapiro.test(bd$`QPVS25D72/D0`[bd$CYP2D6POR == "AM"]) 
shapiro.test(bd$`QPVS25D72/D0`[bd$CYP2D6POR == "AN"]) 
shapiro.test(bd$`QPVS25D72/D0`[bd$CYP2D6POR == "NM"]) 
shapiro.test(bd$`QPVS25D72/D0`[bd$CYP2D6POR == "NN"]) 
```
...
Medidas resumos da quantidade de parasitos no momento inicial com relação ao status de metabolização conjunto (CYP2D6 e Por); status de metabolização CYP2D6; e status de mutação do gene POR.
...
Com o propósito de visualizar se as variáveis Sexo e Idade são fatores de confusão nos resultados obtidos no item 1:
...
Visando testar estatisticamente se exite uma associação entre as variáveis, postulou-se testes estatísticos, cujos resultados foram apresentados na Tabela 8. As hipóteses de interesse dos testes realizados foram:
...
pvalornorm444= norm44$p.value 
```

Além disso, com o propósito de avaliar a contribuição das variáveis Sexo e Idade para o modelo de regressão múltipla do item 1:
...
Assumimos como hipóteses de interesse:
...
Primeiramente foi verificado, através de um diagrama de dispersão, se as variáveis Concentração de primaquina (ng/mL; fármaco estudado) em 72 horas e Razão de Pvs 25  apresentam algum padrão de linearidade, pois caso se confirme pode-se utilizar o ceficiente de correlação linear de Pearson para mensurar tal correlação. 
...
Foi orientado também conduzir a análise tendo em vista a relação das variáveis: Concentração de primaquina (ng/mL; fármaco estudado) em 72 horas e Presença/ausência de gametócitos nos momentos 0 horas e 72 horas. Nesse sentido, a Tabela 12 foi construída visando obter uma descrição de como a variável Concentração de primaquina- 72 horas se distribui, tendo em vista os níveis das vaiáveis: Status de Pvs 25 no momento inicial e Status de Pvs 25 após 72 horas. 
...
pppvaluewill2 = will2$p.value 
metwill2 = will2$method
